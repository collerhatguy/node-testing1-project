...
function findLargestInteger(integers) {
  const largest = integers.reduce((currentLargest, { integer }) => {
    if (currentLargest > integer) return currentLargest
    return integer
  })
  return largest
}
...
class Car {
   ...
   */
  drive(distance) {
    const gasRemoved = distance / this.mpg
    if (this.tank - gasRemoved < 0) {
      const distanceTraveled = this.tank * this.mpg
      this.odometer += distanceTraveled
      this.tank = 0
      return this.odometer
    }
    this.tank -= gasRemoved
